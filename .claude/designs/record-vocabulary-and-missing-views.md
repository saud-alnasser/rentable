...
- **Motion responds or does not happen** ([ADR 0016](../decisions/0016-motion-responds-and-uses-what-is-installed.md),
  [ADR 0021](../decisions/0021-reduced-motion-is-guarded-once-and-loops-are-exempt.md)). A ring
  that fills when its data arrives is a trigger and may animate; one that fills on a timer is a
  loop and may not.
- **One bounded query per state** ([ADR 0010](../decisions/0010-lists-load-whole-result-sets.md)).
...
- **Reconcile owns the derived columns.** The fill reads the materialized paid and expected
  amounts ([ADR 0006](../decisions/0006-payment-aggregates-are-materialized.md)); nothing here
  recomputes a status or an aggregate at render time.
- **Forms keep the shared surface and the field layer**
  ([ADR 0017](../decisions/0017-a-form-surface-is-one-component.md)). Weight, presentation,
...
- **Generated primitives are edited by hand, never regenerated**
  ([ADR 0007](../decisions/0007-rhea-geometry-is-hand-ported.md)). A genuinely new primitive may
...
looked at (`.claude/evidence/prototypes/status-glyphs-and-the-payment-ring.md`) — an arc with a
...
[ADR 0020](../decisions/0020-surfaces-diverge-by-kind-not-by-operation.md) already predicts for
...
([ADR 0025](../decisions/0025-a-row-opens-its-record-and-does-nothing-else.md)). Three surfaces
...
filter twenty in the browser. `.claude/evidence/research/interface-performance-baseline.md`
measured it, [ADR 0010](../decisions/0010-lists-load-whole-result-sets.md) explicitly declined
...
increment: `.claude/evidence/prototypes/status-glyphs-and-the-payment-ring.md`. Its two
...
- **The landing screen's shape.** It renders the new status treatment and its rows navigate, and
  ...
  record is `.claude/evidence/discussions/the-dashboard-after-the-queues-action.md`, and it is a
