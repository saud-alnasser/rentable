...
- **A write that must not half-apply issues one batch** — the boundary runs a batch inside a
  transaction ([ADR 0027](../decisions/0027-a-write-that-spans-tables-is-one-batch.md)).
- **Routers keep reaching the database directly** — no abstraction is added at that seam
  ([ADR 0002](../decisions/0002-no-repository-layer.md)).
- Blue is the only accent; the primitives are a permanent fork and are edited by hand, never
  regenerated ([ADR 0007](../decisions/0007-rhea-geometry-is-hand-ported.md)).
- Lists load whole result sets ([ADR 0010](../decisions/0010-lists-load-whole-result-sets.md)),
...
[ADR 0028](../decisions/0028-a-mutation-is-declared-once-on-the-caller-side.md) records the
...
[ADR 0026](../decisions/0026-undo-is-a-session-stack-of-inverses.md) carries the reasoning and
...
row already does ([ADR 0025](../decisions/0025-a-row-opens-its-record-and-does-nothing-else.md)).
...
[ADR 0024](../decisions/0024-units-read-as-a-directory-and-assigning-them-is-a-form.md) is
...
one batch; carries the correction to the persistence Context that
`.claude/evidence/drift/the-batch-transport-is-a-transaction.md` records, because this is the
change that moves the statement's practical half.
