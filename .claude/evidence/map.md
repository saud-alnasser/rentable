# Evidence map

| Finding | Kind | Falsifies |
| --- | --- | --- |
| [adr-0009-miscites-adr-0007](drift/adr-0009-miscites-adr-0007.md) | drift | `.claude/decisions/0009-the-shipping-list-block-is-rewritten-under-a-new-name.md` |
| [adr-0016-primitive-reduced-motion-is-now-known](drift/adr-0016-primitive-reduced-motion-is-now-known.md) | drift | `.claude/decisions/0016-motion-responds-and-uses-what-is-installed.md` |
| [contract-form-interior](prototypes/contract-form-interior.md) | prototypes | — |
| [contract-queue-group-header](prototypes/contract-queue-group-header.md) | prototypes | — |
| [dependency-backlog-consolidation](research/dependency-backlog-consolidation.md) | research | — |
| [google-drive-conditional-update](research/google-drive-conditional-update.md) | research | — |
| [interface-performance-baseline](research/interface-performance-baseline.md) | research | `.claude/decisions/0008-one-list-block-replaces-both.md` |
| [rhea-geometry-scale](research/rhea-geometry-scale.md) | research | — |
| [svelte-motion-mechanism](research/svelte-motion-mechanism.md) | research | — |
