...
design decisions. It sits at `.claude/position/design/refactoring-ui.pdf`, with a
navigation file beside it at `.claude/position/design/refactoring-ui.md`.
...
`frontend.md`, an ADR, or a Domain Context, this repository wins and the book is not
argued with — precedence is `precedence.md`'s, and a reference ranks below all of it.
...
citation. `engineering.md` binds this everywhere, and it binds here.
...
standards are the only home for them: **the book adds nothing to a question `frontend.md`
or an ADR already answers, and is not consulted on one.** A standard with two homes drifts
at one of them.

## When it is absent

The book and its navigation file are per-clone — `.claude/.gitignore` keeps the whole
