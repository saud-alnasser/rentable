...
- [ ] The preview opens on the application's language, and switching the toggle redraws the page in
      the other (criterion 10(a), component test).
- [ ] The printed schedule and the receipt each take one locale: an Arabic page carries no English
...
- [ ] Each page reads as a document: the issuer at the head's start, title, number and date at its
      end, then the facts; spacing on the ladder of [[rules/frontend]] (component test on structure).
- [ ] *Save as PDF* asks for a path through the save dialog, then prints with `mode: 'pdf'`; *print*
      prints with `mode: 'print'`; a refusal is one sentence (host test).
- [ ] [[rules/interface]], *Print*, and [[rules/frontend]], *i18n*, say what is now true.
