...
- [ ] `member` carries `session_epoch INTEGER NOT NULL DEFAULT 0` outside the member
...
- [ ] `session::end_elsewhere(store, session, now)` bumps the caller's row and pushes, moves
...
- [ ] `session::end_member_sessions(store, session, member_id, now)` requires `ResetPassword`,
...
- [ ] The command the sync heartbeat calls (read `sync/autosync.ts` for which) pulls the
...
- [ ] The you section carries a "sign out of other machines" control with a confirm dialog,
