...
- [x] `member_create` takes `role_id` and `override`, refuses a role not strictly below the actor,
      and refuses an override or role carrying a flag the actor lacks (criterion 7).
- [x] The new member's certificate is issued from the actor's, with the ceiling and rank of their
      effective permissions; no organization key is derived.
- [x] A manager resets a member's password with no organization key derivable in the test; the
      member's fresh certificate verifies and the rows their old one signed still verify.
- [x] A manager removes a member; rows signed under the removed member's certificate afterwards are
      refused; rows signed before verify (criterion 9).
- [x] A removal whose departing certificate signed a row the actor cannot sign is refused, naming
      the flag.
- [x] Lock-out stays the root's; the `session.role` snapshot checks in these files read the verified
      row.
