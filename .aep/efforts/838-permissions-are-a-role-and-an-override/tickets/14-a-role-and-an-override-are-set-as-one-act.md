...
- [x] `member_assign_role` takes an optional override, and with one given, "flags held" compares the
...
- [x] A Rust test gives a member a role whose mask carries a flag the actor lacks together with an
...
- [x] The member card saves a changed role and a changed override in one call, with a component test
