...
- [x] `FLAGS` holds bits 0 to 39 exactly as [[efforts/838-permissions-are-a-role-and-an-override/plan]], *Components*, lists them; `changeRole` is an alias
...
- [x] The 53-bit guard test still fails a flag at bit 53, and a test holds every flag below it.
- [x] `FAMILIES`, `OWNER_ONLY`, `MEMBER_ADMINISTRATION`, `WRITE_FLAGS` and `BUILT_IN` export, with the
      masks and ranks the plan gives.
- [x] `effective`, `effectiveIn`, `xorOf`, `maskOf` and `permits` are arithmetic; a test at bit 39
      shows `xorOf` and `permits` right where `^` and `&` would coerce to 32 bits.
- [x] A JSON table of `(mask, override, effective)` cases, checked in once, is read by a TS test and
      a Rust test, and both pass (criterion 6).
- [x] The Rust test that reads the package source asserts the names, bits, families and built-in
