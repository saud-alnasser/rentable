...
- [ ] `error/toast.ts` `showErrorToast` no longer shows `toErrorMessage`'s `detail` as visible
      ...
      room.
- [ ] Tests render a non-refusal shell failure in Arabic, through a toast and through
      `toErrorText`, and assert the shell's English message is not visible text.
- [ ] `[[rules/interface]]` and `[[rules/api-layer]]` still describe what the code does.
