...
Everyday git — `add`, `commit`, `log`, `diff`, `checkout` — needs no entry here. This file covers the reads the workflow depends on, the parsing that quietly goes wrong, and the standing rules about writing history.
