...
produces no branch, so it never becomes a tracker item, and it lives in its effort's
`spec.md` instead. What it still needs is a route onto `main`.
...
frontend build, and a release-profile compile of the Rust binary. All of it must pass. **It
is the only required check** — the title lint reports separately and, until it is added to
the default-branch ruleset, a failing title does not block the merge.
...
- **Nothing in the commit body reaches `main`.** The body is still worth writing: it is
  ...
  the workflow controls that can reach the pull request at all. But a closing keyword only
  closes its issue by way of the pull request body, never by riding the commit onto trunk.
