<script lang="ts">
	import * as Sheet from '#lib/primitive/sheet/index.js';
	...
	>
			...
			>
				...
				>
					<Sheet.Header class="sr-only">
						<Sheet.Title>{contract.strings.sidebar}</Sheet.Title>
						<Sheet.Description>{contract.strings.mobileSidebarDescription}</Sheet.Description>
					</Sheet.Header>
		...
		{:else}
			...
			<div
				...
				class={cn(
					'fixed inset-y-0 z-10 flex h-svh w-(--sidebar-width) transition-[left,right,width] duration-base ease-move',
					side === 'left'
			...
			>
				<div
					...
					class="flex h-full w-full flex-col bg-sidebar group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:border-sidebar-border group-data-[variant=floating]:shadow-raised"
				>
					{@render children?.()}
				</div>
			</div>
