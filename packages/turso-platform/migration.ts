...
/**
 * Bringing a **hosted workspace database** up to a schema version, over the wire.
 *
 * **This file talks to exactly one database**: the Turso database one workspace's ledger lives
 * in, reached over libSQL with a token minted for it, and touched only to apply DDL. No row of
 * anybody's ledger is read or written here; applying a schema is not the data path.
 *
 * **Kept, not used.** It was the control plane's runner and the only one there was while
 * requirement 11 of the hosted-workspace effort kept the desktop's away. Organizations gave the
 * desktop a runner of its own in Rust (`apps/desktop/tauri/src/organization/migrate.rs`), and
 * whichever member opens a workspace applies what is pending under a lease. This is the same
 * semantics in TypeScript, for a hosted tier that remains possible without being planned; the
 * desktop imports none of it.
 */

/**
 * The workspace migrations this package ships, which are the same ones the desktop ships.
 *
 ...
 * every consumer depends on rather than a directory inside any of them. *This replaced a copy
 * under the control plane, which was wrong twice: the guard holding the two directories identical
 ...
 * migrations were that second consumer.*
 */
const FOLDER = workspaceMigrationsFolder;

/**
 * What has been applied to a workspace database, kept in the database it was applied to.
 *
 * **The ledger is the authority and any record elsewhere is an index of it**: a run that failed
 * partway leaves the two disagreeing, and the one that was written by the statements that
 * actually ran is the one to believe.
 *
 * *It is shaped exactly as the desktop's own runner once wrote it, same table, same columns,
 * same key, because that runner used to read it off the replica to decide what to apply. The
 * desktop's runner today records the version on the organization database instead, so the shape
 * is inherited rather than owed to anybody.*
 */
const LEDGER =
 ...
 *
 * **A port, for the same reason `TursoPlatform` is one**: the tests run the real migration code
 * against a real libSQL database on a temporary file, which is the same client type a deployed
 * service would open against Turso, and only that service's wiring would put a real hostname
 * and a real token into it.
 */
export type ConnectToWorkspaceDatabase = (where: { url: string; authToken: string }) => Client;

/**
 * How long the token a runner opens a workspace database with lives.
 *
 * **Minutes, and deliberately unlike a person's session.** It is held by the process for the
 * length of one migration and never leaves it, so its window is the work rather than a person's
 * absence. The desktop's lease on a pending migration has the same lifetime, for the same
 * reason.
 */
