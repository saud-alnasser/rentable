...
/**
 * Turso's Platform API, as the control plane used it.
 *
 * **A port, not a client.** Everything below is reached through this shape, so a service built
 * on it runs its routes against a Turso that answers in memory, and the one place a live account
 * is touched is the wiring that constructs this. A service whose tests need somebody's cloud
 * account is a service nobody runs the tests of.
 *
 * **Kept, not used.** The control plane retired when organizations arrived
 * ([[efforts/819-an-organization-hosts-its-own-workspaces/spec]], requirement 19); the desktop
 * reaches this API through its own Rust client, which holds the customer's own authority. This
 * is what was known about the API in TypeScript, so a hosted tier remains possible later
 * without being planned now. It is on no credential path and nothing imports it.
 *
