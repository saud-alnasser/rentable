...
`vacant`. That status is *derived*, never authored — see **Reconcile**.
...
**Cost** — the amount owed *per interval*, not the contract total. The column is
...
- *Status reconciliation* — recompute contract and unit status from dates and payments.
  Must run after any mutation touching contracts, payments, or assignments, or statuses
  go stale.
- *Manifest reconciliation* — recompute a backup manifest from the snapshot files actually
  present on disk.
...
**Sync** — always means *remote*: exchanging workspace state with Google Drive. Never used
...
that performs it. A workspace is *linked* or *unlinked*.
...
decision. Distinct from an *overlap*, which is two contracts competing for one unit.
...
Recorded in full in `CLAUDE.md`. In brief:

- Google Drive HTTP and OAuth live in Rust. Credentials never cross the IPC boundary.
- Domain rules live in the concept's `domain.ts`. There is no repository layer; routers
  use Drizzle directly.
- Modules are organised by concept, not by layer. `src/routes/` is the exception.
