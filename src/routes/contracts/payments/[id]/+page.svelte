<script lang="ts">
	...
	import ContractPaymentsDataView from '$lib/resources/contracts/components/contract-payments-data-view.svelte';
...
<div class="mx-auto flex min-h-0 w-full max-w-7xl flex-1 flex-col gap-4 p-5">
			...
			<Tooltip.Root>
				<Tooltip.Trigger>
					{#snippet child({ props })}
						<Button
							{...props}
							href={resolve(`/contracts/${contractId}`)}
							variant="outline"
							size="icon-sm"
	...
	<ContractPaymentsDataView {contractId} />
</div>
