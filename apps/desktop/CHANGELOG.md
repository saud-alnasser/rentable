...
- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - a new contract chooses its units on the contract form, beside the tenant, and creating it assigns them in the same step. only units no other contract holds over the chosen term are offered, a unit taken in the meantime is refused under the units field, and undo takes back the contract and its units together. the units tab stays for changing them later

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - a tenant's page and a unit's page each offer "new contract", in their actions and on their contracts list, and the contract form opens with that tenant or that unit already chosen. a chosen unit that another contract holds over the term you pick stays in the units list, checked, and says so, so you can take it back out before saving

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - creating an organization is two steps: connect your turso account, then name the organization and choose your username and password. the first workspace is made for you, named after the organization, as the first stage of the one loading screen that follows, and you can rename it later. the connect step says one line, with what to know before connecting behind "before you connect"

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - render english and arabic in readex pro, bundled with the app, on one type scale with arabic given the line height it needs

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - deleting a tenant, a complex, a unit, a payment or a contract happens at once, and the message that follows offers undo and says it lasts while the app is open. a record something still depends on is refused as before, naming what holds it. deleting a workspace or removing a member still asks first, and terminating or restoring a contract, signing out elsewhere, forgetting the account and disconnecting now ask in a dialog named for what they do rather than in the delete dialog

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - joining goes from the password straight to the one loading screen, and the screen you joined from is never drawn again once it is up. every refusal on the way in says what happened and what to do next in one line, with what rentable was told behind "details"

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - rentable now draws in light as well as dark, and follows your system by default: when your computer switches between light and dark, rentable switches with it, with no restart. to keep one of them regardless, general settings has a new appearance choice of system, light or dark, which applies the moment you press it and is remembered the next time rentable opens, before its window appears. toasts follow the same appearance

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - lists, records, settings and the dashboard load the same way: a skeleton shaped like what is coming, shown only when a load takes a moment. The note on a locked contract's units now reads as a notice rather than an error

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - search every list the same way, the contract's unit panes and the settings members and workspaces included: a search glass, a short wait after typing, and / to jump to the field. members and workspaces can also be sorted

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - the app fills what it can and lands you where the next step is: a new payment opens on today and on what is due this cycle, a new contract opens its own page, and a new tenant, complex or payment is brought into view in its list with the focus on it. an act that cannot run right now stays in place, dimmed, and says why in one line when you hover or focus it, so a paid-in-full or terminated contract's add-payment control says why instead of a paragraph above the ledger

### Patch Changes

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - a contract that has taken no payment deletes at once with the units it holds, which read vacant again, and undo puts it back holding them. a selection of contracts does the same. every form now says a refusal it cannot place beside a field, such as a payment on a contract paid in full, instead of closing on nothing. duplicating a payment is refused on a contract paid in full, as a new one is. a permission failure reads as its own sentence everywhere, and an undo that fails part way leaves the screen showing what was written

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - a unit's page shows its complex in the trail at the top, between complexes and the unit, and clicking it opens that complex

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - a contract offers the same acts, in the same order and under the same names, on its card, its right-click menu, its page and the command palette: cards gain copy details and duplicate, the palette can now run every contract act, not only renew, and a card's menu separates the acts that change a contract's standing and the one that deletes it

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - directories move with what you do: a record you create slides in, one you delete leaves, an undone delete comes back in its place, and a new sort carries each record to where it now belongs. typing a search still narrows the list at once, and reduced motion keeps every change instant

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - an empty list says what it will hold and offers to add the first record, and a search or filter that matches nothing says so and offers to clear it, so the two no longer read alike. the contract's unit panes, the dashboard and the settings directories use the same treatment. a record that no longer exists says so and offers the way back, and an address that leads nowhere says the page does not exist instead of that the screen could not be shown

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - a screen that failed to load or to draw says what happened in your language, with the technical message behind "details". a record you create is brought into view only where it is on screen at the time, so opening its list later or clearing a filter no longer jumps to it. two lists changing together, such as a contract's payments and its history, now each move in their own frame without cutting the other short

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - a failure that is not a refusal now reads in your language too: an act your role does not allow, a signed-out machine, something entered that is not valid, and an unexpected failure each say a short sentence of their own, and something entered that is not valid is placed under its field where the form has one

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - every list can be sorted, a complex's units and a contract's payments included. the contract's unit panes search on the same bar as every other list and show each unit as the same card, with its menu. when a set takes no new records, the "new record" in its empty state is dimmed and says why, as the toolbar's does. a record that no longer exists and an address that leads nowhere look the same and offer one way back. the members section orders its toolbar like every other list, and its descriptions are muted

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - a member's role, a workspace's access and the language are chosen from buttons side by side rather than from a menu, the way the appearance already was. the member's sheet says what the chosen role and access mean under each control, and a choice only the owner may make is still shown dimmed for anybody else

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - figures read in western digits in arabic as in english: amounts, counts, dates, relative times and the calendar in the date pickers, which now names its months in the reader's language. a search typed in arabic-indic digits still finds what it did

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - forms speak one way: a complex opens on the side panel for editing as well as creating, a contract's cycle is four buttons side by side instead of a menu, a cost or a payment amount shows the riyal sign and brings up the number keypad, the payment date's calendar keeps clear of the window's edge, and every submit carries a glyph with its verb. a refused submit now takes you to the first field that needs fixing

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - a member's card in settings offers one edit where it offered rename and edit side by side: the member's name is now the first section of the sheet the edit opens, beside their role and workspaces. a workspace's card says edit where it said rename, and both carry the same edit glyph as every other record. the menus separate the acts that open a way in and the ones that remove somebody

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - the command palette offers a member's and a workspace's acts, as it offers every other record's: it lists only the acts you may take, then only the members or workspaces the chosen act applies to, and runs it through the same settings surfaces the cards open

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - motion runs on one short, consistent set of speeds; the command palette opens at once, and reduced motion stops every slide, zoom and fade

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - a name or an address written in Latin letters now keeps its order on an Arabic screen, so "Adeline Wiegand Sr." no longer reads ".Adeline Wiegand Sr" and "4253 Russel Motorway" no longer reads "Russel Motorway 4253", on the lists, a record's title and its fields, and the breadcrumb. a disabled button is dimmed in a colour you can still read in both light and dark, rather than faded to grey on dark blue. the section tabs no longer show a scrollbar when nothing scrolls, and a long list's scrollbar no longer shrinks to a dot

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - the breadcrumb links only to pages that exist and ends on the record you are reading, by its name. a record's sections and the settings sections switch with one row of links, so every section has its own address, a contract's history included. the organization walk and the join screen go back with the same control as a record's page

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - every icon now comes from one family and one set of sizes, so the same act wears the same glyph on every screen, and editing and renaming share one. in arabic, a record's back control sits at the right edge again, where its arrow points

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - every list and settings directory offers its create in the same place, at the end of the bar above the records, and ctrl or cmd with n creates in the one on screen. the command palette now creates units and payments too, asking first for the complex or the contract they belong to, and the dashboard's renew is the same control a contract's card and page offer

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - the organization and sync refusals read in arabic as in english: signing in, members, workspaces, links and the first run say why in your language, and what turso itself said sits behind a details control rather than inside the sentence

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - tenants, complexes, units and payments offer the same acts, in the same order and under the same names, on their cards, their right-click menus, their pages and the command palette: a unit's page gains edit and delete, cards gain copy details (and a payment's card gains duplicate), and the palette can run any of them on a record it finds

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - refusals read in arabic as in english: what the app says when it turns a change away, and where a form puts it, no longer depends on english wording. a refusal from the organization setup that went unrecognised is recognised again

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - corners and shadows come from one small named set, and a card, menu or dialog stands off the page by the same height in light as in dark

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - The dashboard's occupied units card carries its label at the top, so its ring lines up with the collected ring beside it.

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - a unit's status reads by its shape as every status does, a set with nothing in it shows no count above its empty state, the payment page names its contract in the trail and shows its locked acts with their reason, export and import say why they cannot run, a failed page offers retry, and Turso is written with its capital

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - counts read as real plurals in english ("1 member", "103 contracts"), an empty list's create names what it makes ("new tenant"), and a title never capitalises "and" or a workspace's own name. settings headings share one case, a contract's period is written with the same dash everywhere, and english writes "ID"

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - a failure nobody can act on, such as a file that could not be read, now says its sentence in the reader's language and nothing else. the shell's own english words no longer appear in the toast or beside the sentence: the sign-in screen, the sync standing and the update recovery screen keep them behind details, and everywhere else they go to the diagnostics log

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - the sheet that adds a member reads like the sheet that edits one: the username under its own line, the role in its tray, what they may do beyond their role as a list you add to, and a row per workspace where no access means not granted. a segment of a segmented control keeps its side padding however narrow the row, so "administrator" no longer touches its edges

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - a contract's units tab now lists every unit the contract holds, including one that another contract running over the same dates also holds. before, such a unit was left off the tab, so deleting the contract could be refused for units the tab did not show; the count a refusal gives is now the count the tab lists

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - the words are shorter and plainer in english and arabic: descriptions, notices and refusals say what matters in a line, and only the disconnect dialog still spells out its whole consequence. no sentence names the currency; the amount carries the riyal sign

- [#833](https://github.com/saud-alnasser/rentable/pull/833) [`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - undoing a contract's deletion puts it back as it was: a terminated contract comes back terminated, holding the units it held, even where another contract has taken one of them since. undoing a new contract is one step that cannot stop halfway, and an import that fails says so once
- Updated dependencies [[`95fa36b`](https://github.com/saud-alnasser/rentable/commit/95fa36bd9a70954abeaf24b91df966db173ac999)]:
  - @rentable/design@0.0.1
