...
- [#820](https://github.com/saud-alnasser/rentable/pull/820) [`4b7f2dc`](https://github.com/saud-alnasser/rentable/commit/4b7f2dc2ff880fc0ba22d6a5e077e58096f28034) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - a member invited or reset on a generated password chooses their own before anything else opens, changes it again from the account page whenever they like, and an administrator resets a password they do not know from the organization page, told which workspaces the reset could not restore
...
- [#820](https://github.com/saud-alnasser/rentable/pull/820) [`4b7f2dc`](https://github.com/saud-alnasser/rentable/commit/4b7f2dc2ff880fc0ba22d6a5e077e58096f28034) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - a member joins an organization by opening the invitation link they were handed, which opens the application on windows, linux and macos, and by pasting it into the join screen where it does not; the organization is added to the sign-in screen's list, the generated password opens their place in it, and a lapsed, used or revoked invitation is refused with the organization still named
...
- [#820](https://github.com/saud-alnasser/rentable/pull/820) [`4b7f2dc`](https://github.com/saud-alnasser/rentable/commit/4b7f2dc2ff880fc0ba22d6a5e077e58096f28034) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - signing in is a password that unlocks your place in an organization, on this machine, with or without a connection; the sign-in screen lists the organizations this machine has joined and no longer offers google
...
- [#820](https://github.com/saud-alnasser/rentable/pull/820) [`4b7f2dc`](https://github.com/saud-alnasser/rentable/commit/4b7f2dc2ff880fc0ba22d6a5e077e58096f28034) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - an owner or a member gets their organization back on a new machine from the organization's own link and their password, with everything they held restored from the organization database and verified; an owner then grants the turso consent again from the organization page, because the authority is kept on no database
...
- [#825](https://github.com/saud-alnasser/rentable/pull/825) [`9960b46`](https://github.com/saud-alnasser/rentable/commit/9960b4685a622fa4af3fd6b40ca34c3a4df9b014) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - every screen that names a member does so by username. setting up an organization asks the owner for theirs on the same step as the name and the password; inviting somebody asks for a username, a role and the workspaces, and what it shows afterwards is the organization's link, the username and the generated password, each with its own copy control, since the application sends nothing itself. the members list, the pending accounts list and the account page name people by username alone, the organization page says that the link with a username and a password is the way in, and it offers to disconnect this machine from the organization after one confirm, which signs you out and forgets every copy kept here while leaving the organization on turso untouched
...
- [#820](https://github.com/saud-alnasser/rentable/pull/820) [`4b7f2dc`](https://github.com/saud-alnasser/rentable/commit/4b7f2dc2ff880fc0ba22d6a5e077e58096f28034) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - an administrator invites a member from the organization page, naming their role and the workspaces they belong to, and hands over the link and the generated password themselves because the application sends no mail; an invitation lapses after a week, can be revoked while unused, and can be reissued, which is what a reset is
...
- [#825](https://github.com/saud-alnasser/rentable/pull/825) [`9960b46`](https://github.com/saud-alnasser/rentable/commit/9960b4685a622fa4af3fd6b40ca34c3a4df9b014) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - inviting a member and creating a workspace are dialogs now, opened from the workspace menu at the top of the sidebar and from the organization page alike, where before they were forms sitting between the page's lists. the menu's two rows are live for whoever may use them and say whose act it is for everybody else, with no padlock; the invite dialog keeps the link and the generated password on screen until you press done, and the email, name and workspace fields carry a glyph for what they take

- [#827](https://github.com/saud-alnasser/rentable/pull/827) [`f56ac57`](https://github.com/saud-alnasser/rentable/commit/f56ac57af993d56a0dc82cc7b1332a3112b71de8) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - settings is one area with seven sections rather than four pages. `/organization`, `/workspace` and `/account` are gone, and `/settings` holds general, you, members, workspaces, sync, updates and diagnostics as tabs above the page, each with an address of its own so a menu or the command palette can open one directly. what a section shows is what your place in the organization allows, and a section you have nothing to do in is not offered at all. the rail's workspace menu opens the workspaces section, the account menu offers you and settings, and the page still opens before anybody has signed in, with the language, updates and diagnostics sections on it
...
- [#825](https://github.com/saud-alnasser/rentable/pull/825) [`9960b46`](https://github.com/saud-alnasser/rentable/commit/9960b4685a622fa4af3fd6b40ca34c3a4df9b014) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - signing in is a username and a password against the organization this machine holds. the password is tried against each member's vault and the username on the row that opens has to be the one typed, so a wrong password, an unknown username and a known username with somebody else's password are refused with one sentence; a first sign-in on a handed password spends the invitation and still asks for a password of the member's own, and one whose invitation has lapsed is told so. an invitation is the organization's link, the username and the generated password, handed over together; the link carries no invitation half any more, and joining or restoring by a link are gone with it, since connecting is by the link alone and everyone is admitted at the wall
...
- [#827](https://github.com/saud-alnasser/rentable/pull/827) [`f56ac57`](https://github.com/saud-alnasser/rentable/commit/f56ac57af993d56a0dc82cc7b1332a3112b71de8) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - inviting somebody, and resetting their password, now produce a link to send and a six-character confirmation code to read out on a call. the code is one half of what opens the invited vault and the link's secret is the other, so a link that leaks or is forwarded opens nothing without a code that was alive when it was typed; a code lapses ninety seconds after it is made, the issuer makes a fresh one from the result panel or the pending member's row, and the person opening the link types it above the password they choose. the connect screen no longer names the invited person before the code is typed

- [#827](https://github.com/saud-alnasser/rentable/pull/827) [`f56ac57`](https://github.com/saud-alnasser/rentable/commit/f56ac57af993d56a0dc82cc7b1332a3112b71de8) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - the members section is one list. everybody is a row, the people who were invited and have not signed in yet included: the avatar and the username with the role beside it, the workspaces they hold as chips saying full access or read only, and on somebody who has not arrived a badge saying so with the day their link runs out. the row's actions appear when the pointer or the keyboard reaches it, and you are offered only the ones you may perform: role and permissions, workspaces and access, rename, a new link, remove, and on a pending row the link to copy again where you are the one who issued it, a new link, and revoke. role and permissions opens a panel with the three roles and a checkbox for each act, where giving somebody an act that writes another member's row is the owner's alone and says so; workspaces and access opens a panel offering each workspace at no access, full access or read only, where read only is the owner's and says so. inviting asks for a username, a role and an access per workspace, and what it shows afterwards is the organization's name, the sentence that rentable sends nothing, one link and one control to copy it. a new link and a copy link show the same panel. the separate list of pending accounts is gone.
...
- [#827](https://github.com/saud-alnasser/rentable/pull/827) [`f56ac57`](https://github.com/saud-alnasser/rentable/commit/f56ac57af993d56a0dc82cc7b1332a3112b71de8) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - an invitation is one link. inviting or resetting a member hands you a single link to send, with no password beside it; the person opens the link and chooses their own password, and a link somebody kept opens nothing once it is revoked. the person who issued a link can copy it again; anybody else issues a new one.
...
- [#827](https://github.com/saud-alnasser/rentable/pull/827) [`f56ac57`](https://github.com/saud-alnasser/rentable/commit/f56ac57af993d56a0dc82cc7b1332a3112b71de8) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - the connect screen takes either link in one field: an organization link records the organization on this machine and lands on the sign-in, and an invitation link records it, names the organization and your username, and asks you to choose the password you will sign in with from then on. a link that has lapsed or was withdrawn says to ask whoever invited you for a new one, a link for another organization says to disconnect this machine first, and a link that was already opened says so and offers the sign-in, since a second machine is connected by the same link and the password you chose is what opens it. the sign-in screen of a connected machine now offers the connect screen too, for a reset link
...
- [#825](https://github.com/saud-alnasser/rentable/pull/825) [`9960b46`](https://github.com/saud-alnasser/rentable/commit/9960b4685a622fa4af3fd6b40ca34c3a4df9b014) Thanks [@saud-alnasser](https://github.com/saud-alnasser)! - connect to an organization by its link alone: the link screen records the organization on this machine and lands on the sign-in wall, and no longer asks for a password or a place to restore
