// Shared scaffolding for the lint tests that read this application's source as text. Not a
// `*.test.ts` file, so the test runner does not pick it up directly.
//
// `packages/design/src/tests/source.ts` is the package's copy. Each package scans its own tree
...
/** this application's `src/`, which every label is written from. */
