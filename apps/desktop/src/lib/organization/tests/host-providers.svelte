<script lang="ts">
	 ...
	 * the reason `src/tests/query-providers.svelte` gives. A directory's cards ask the host for what
	 * their acts open, and the host is mounted once in the frame rather than by the directory, so a
	 * test of what a card's act opens renders the two side by side, the way the shell draws them.
	 * The host's hooks are the test's to stand in for: `./host-hooks.ts` holds the stand-ins.
	 */
