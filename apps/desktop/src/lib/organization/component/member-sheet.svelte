<script lang="ts" module>
	/** what the sheet hands back on a save: the role, the acts, and the grants that changed. */
...
<script lang="ts">
	 ...
	 *
	 * **What each section is drawn for is what this reader may write.** The role and the widening
	 * are `changeRole`'s and the workspaces are `grantWorkspace`'s (effort 826, requirement 15),
	 * so a reader holding one of the two meets one section rather than a surface of controls that
	 * refuse them.
	 *
