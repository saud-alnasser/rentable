 ...
 * @property {{ id: string, name: string, ownerAccountId: string, createdAt: number, updatedAt: number }} [workspace]
 ...
 * @param {(reached: { url: string; db: import('./database.ts').Database; turso: ReturnType<typeof tursoInMemory> }) => Promise<void>} run
 * @param {ReturnType<typeof tursoInMemory>} [turso]
 */
const withControlPlane = async (verifyIdentity, run, turso = tursoInMemory()) => {
 ...
 * @param {string} url
 * @param {string} path
 * @param {{ token?: string | null, body?: unknown }} [how]
 */
const post = (url, path, { token = 'a-token', body } = {}) =>
	fetch(`${url}${path}`, {
		method: 'POST',
		headers: {
