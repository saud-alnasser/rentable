...
/**
 * @param {import('./google.ts').VerifyGoogleIdentity} verify
 * @param {string} token
 * @returns {Promise<Refusal>}
 */
const refusalFrom = async (verify, token = 'a-token') => {
...
test('the token is presented to Google as a bearer credential', async () => {
	/** @type {{ url: unknown; headers: Record<string, string> } | undefined} */
	let seen;

	const verify = verifyAgainstGoogle({
		endpoint: 'https://example.test/userinfo',
		fetch: async (url, init) => {
			seen = { url, headers: /** @type {Record<string, string>} */ (init?.headers ?? {}) };
